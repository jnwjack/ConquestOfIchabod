...
WEB_FLAGS_LIBS = -sUSE_SDL=2 -sUSE_SDL_IMAGE=2 -sUSE_SDL_TTF=2 -sSDL2_IMAGE_FORMATS=["png"] -sUSE_SDL_MIXER=2
WEB_FLAGS_LIBS2 = --use-port=sdl2_image:formats=png --use-port=sdl2_ttf
WEB_FILES = --preload-file=assets --preload-file=test_data --preload-file=src/engine/etc --preload-file=src/threadtown/spritemap.dat --preload-file=src/armory/spritemap.dat  --preload-file=src/battle/spritemap.dat --preload-file=src/threadtown/spritemap_path.dat --preload-file=src/threadtown/spritemap_corrupt_1.dat --preload-file=src/threadtown/spritemap_corrupt_2.dat --preload-file=src/threadtown/spritemap_corrupt_3.dat --preload-file=src/threadtown/spritemap_corrupt_4.dat --preload-file=src/threadtown/spritemap_corrupt_1_path.dat --preload-file=src/threadtown/spritemap_corrupt_2_path.dat --preload-file=src/threadtown/spritemap_corrupt_3_path.dat --preload-file=src/threadtown/spritemap_corrupt_4_path.dat
SRCS = $(shell find src -name "*.c")
OUTPUT = coi
WEB_OUTPUT = coi.html
LIBS = -lSDL2 -lSDL2main -lSDL2_image -lSDL2_ttf -lSDL2_mixer -lm
...
web:
	rm -rf web_output
	mkdir web_output
	mkdir web_output/assets
	mkdir web_output/sound
	mkdir web_output/src
	mkdir web_output/src/engine
	mkdir web_output/src/engine/etc
	$(WEB_CC) $(SRCS) $(WEB_FILES) $(WEB_FLAGS) $(WEB_FLAGS_LIBS) -o $(WEB_OUTPUT)
	cp sound/*.wav web_output/sound/
	cp assets/*.png web_output/assets/
	cp src/engine/etc/* web_output/src/engine/etc/
	mv coi.data web_output/coi.data
	mv coi.html web_output/index.html
	mv coi.js web_output/coi.js
	mv coi.wasm web_output/coi.wasm
